...
export class PlayComponent implements OnInit {
    ...
    lremoveQuote(e: MouseEvent, i: number){
        this.room.quotes.splice(i, 1);
        this.room.quotes.pop;            

    } 
    
    removeQuote(e: MouseEvent, quote: Quote, i: number){
        e.preventDefault();
        const data = { text: quote.text, player: this.me.name };
        this.http.post(this.game.apiRoot + "/game/room/removedQuotes", data).subscribe(res=>{
            this.room.quotes.splice(i, 1);
            this.me.quotes.push( res.json() )          
        })
    }    
